...
# All class IDs YOLO can express an opinion on (Y or N). Used to pre-populate
# an "N" row for every mapped class YOLO does NOT detect in an image, so
# absence is recorded directly from the model instead of via a GPT guess.
ACTIVATION_CLASS_IDS = set(ACTIVATION_MAPPINGS.values())
...
def process_single_image(
    ...
    For every class ID in ACTIVATION_CLASS_IDS, YOLO now makes a direct,
    authoritative call: "Y" if it detected that class in the image, "N" if
    it did not. Absence is no longer inferred by sending the image to GPT —
    a class YOLO's model doesn't see is recorded as N straight away.

    Returns
    -------
    results        : list[dict]   — result rows for DB/CSV (one Y/N row per
                                     mapped class ID, when a model ran)
    mapped_ids     : set[int]     — mapped class IDs YOLO has an opinion on
                                     (both Y and N)
    others_detected: bool         — True if 'others' (unmapped) class appeared
    zero_detection : bool         — True only when YOLO never ran at all
                                     (no model configured / image unreadable)
    annot_bgr      : ndarray|None
    """
    fileseqid, storename, filename, local_path, s3_key, storeid, subcategory_id = image_info
    ...
    try:
        ...
        if activation_yolo_model:
            # Pre-populate every known class as "N" — absence is the default,
            # authoritative call unless YOLO actually detects the class below.
            yolo_output = {str(cid): "N" for cid in ACTIVATION_CLASS_IDS}
            ...
            n_count = sum(1 for v in yolo_output.values() if v == "N")
            y_count = len(yolo_output) - n_count
            logger.info(
                f"  → YOLO direct Y/N for {filename}: {y_count} Y, {n_count} N "
                f"(of {len(yolo_output)} known classes)"
            )
        else:
            logger.warning(f"  No YOLO model configured — skipping {filename}")
...
def run_yolo_analysis(
    ...
    Y/N determination:
      - Every class ID YOLO knows about (ACTIVATION_CLASS_IDS) gets a direct
        Y/N row from YOLO itself: Y if detected in the image, N if not.
        GPT is never consulted to decide presence/absence for these classes.
      - GPT vision is used ONLY to classify YOLO's generic 'others' catch-all
        detections — i.e. things on the shelf/cooler that don't match any of
        YOLO's known classes. GPT is triggered per image solely when 'others'
        was detected, and any GPT result for a class ID YOLO already decided
        (Y or N) is discarded — YOLO's direct call always wins.
    """
    import pg8000.dbapi as pg
    ...
    for image_info in images_to_process:
        ...
        # 5B. GPT fallback decision — image level only
        #
        # YOLO now makes a direct Y/N call for every class it knows about
        # (see process_single_image), so GPT is no longer needed to infer
        # absence. GPT is triggered ONLY when YOLO's generic 'others'
        # catch-all class was detected — i.e. there's something on the
        # shelf/cooler that doesn't match any known class, and only GPT can
        # say what it is.
        needs_gpt = others_det
        ...
        if needs_gpt:
            if not gpt_available:
                ...
                for detection in gpt_detections:
                    ...
                    # YOLO already made an authoritative Y/N call for every
                    # class ID in ACTIVATION_CLASS_IDS (see process_single_image).
                    # Never let GPT override that — GPT only fills in class IDs
                    # YOLO has no opinion on at all (e.g. GPT_CLASS_MAP entries
                    # like COUNTERTOP_DISPLAY=1077 that aren't in
                    # ACTIVATION_MAPPINGS), which is exactly what's left out of
                    # yolo_inserted_keys.
                    if (filename, cid) in yolo_inserted_keys:
                        logger.info(
                            f"  GPT dedup skip: {filename} classid={cid} "
                            f"already decided directly by YOLO"
                        )
                        continue
