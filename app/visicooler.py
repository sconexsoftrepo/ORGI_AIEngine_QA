...
def extract_brand_from_name(name: str) -> str:
    """
    Extract brand name from product/cap name with FIXED ordering.
    
    CRITICAL FIX: Check specific variants BEFORE general brands
    
    Examples:
        "Coke Zero Cap" -> "coca-cola"
        "Coca-Cola Zero 250ml Glass" -> "coca-cola"
        "Coke Cap" -> "coca-cola"
        "Sprite 250ml PET" -> "sprite"
    """
    name_lower = name.lower().strip()
    ...
    # ========================================
    # CRITICAL FIX: CHECK SPECIFIC VARIANTS FIRST
    # Order matters - check "coca-cola zero" BEFORE "coca-cola"
    # ========================================
    
    # Coca-Cola variants (CHECK ZERO FIRST!)
    if any(x in name_lower for x in ["coca-cola zero", "coca cola zero", "coke zero"]):
        return "coca-cola"
    if any(x in name_lower for x in ["coca-cola", "coca cola", "coke"]):
        return "coca-cola"
    ...
    # Mountain Dew
    if "mountain dew" in name_lower or "mtn dew" in name_lower:
        return "mountain dew"
    
    # Thums Up variants
    if "thums up" in name_lower or "thumbs up" in name_lower or "thumps up" in name_lower:
        return "thums up"
    ...
    # 7Up variants
    if "7up" in name_lower or "7 up" in name_lower or "seven up" in name_lower:
        return "7up"
    ...
    # Maaza
    if "maaza" in name_lower or "maza" in name_lower:
        return "maaza"
    
    # Mirinda
    if "mirinda" in name_lower or "miranda" in name_lower:
        return "mirinda"
    ...
    # Kinley variants (CHECK SPECIFIC TYPES FIRST!)
    if "kinley soda" in name_lower or "kinley club soda" in name_lower:
        return "kinley soda"
    if "kinley water" in name_lower or "kinley mineral water" in name_lower:
        return "kinley water"
    if "kinley" in name_lower:  # Generic Kinley
        return "kinley"
    
    # Water brands
    if "aquafina" in name_lower:
        return "aquafina"
    if "evervess" in name_lower:
        return "evervess"
    if "lehar" in name_lower:
        return "lehar"
    
    # Energy drinks
    if "charged" in name_lower:
        return "charged"
    if "sting" in name_lower:
        return "sting"
    
    # Teem
    if "teem" in name_lower:
        return "teem"
